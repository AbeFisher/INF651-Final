...
const createElemWithText = (ElementName = "p", text = "", ClassName) => {
...
function toggleCommentSection(postId) {
    ...
    const sections = document.querySelectorAll(`section[data-post-id = '${postId}']`);
...
//    e. If a postId exists, 
//       -  add a click event listener to the button (reference
//          addEventListener) - inside the loop so this happens to each button
// f. The listener calls an anonymous function (see cheatsheet)
// g. Inside the anonymous function: 
//    -  the function toggleComments is called with the event and postId as parameters
...
function addButtonListeners() {
    ...
    buttons = main.getElementsByTagName("button");
...
// d. If a postId exists:
//    -  remove the click event listener from the button, inside the loop
//       so this happens to each button (reference removeEventListener)
...
async function getUsers() {
    try {
        ...
        alert(`Error fetching user data.  User list could not be populated.\n\nError Message: \n${err}.`);
    }
}
...
async function getUserPosts(userId) {
    ...
    try {
        ...
        alert(`Error fetching posts for user "${userId}".\n\nError Message: \n${err}.`);
    }
}
...
async function getUser(userId) {
    ...
    try {
        ...
        alert(`Error fetching data for user "${userId}".\n\nError Message: \n${err}.`);
    }
}
...
async function getPostComments(postId) {
    ...
    try {
        ...
        alert(`Error fetching comments for postID "${postId}".\n\nError Message: \n${err}.`);
    }
}
...
async function createPosts(posts) {
    ...
    for (i=0; i<posts.length; i++) {
        ...
        const name = createElemWithText("p", `Author: ${author.name} with ${author.company.name}`);
        ...
        article.append(title, body, id, name, phrase, button, section);
        fragment.append(article)    
    }
